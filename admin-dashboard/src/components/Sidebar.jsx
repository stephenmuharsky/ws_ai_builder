...
export default function Sidebar({ pendingCount = 0, rejectedCount = 0 }) {
  const links = [
    {
      to: '/pending',
  ...
  return (
    <aside className="w-64 bg-surface-light border-r border-navy-100 flex flex-col flex-shrink-0">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        <p className="px-4 text-[10px] font-bold text-navy-300 uppercase tracking-widest mb-2">
          Lead Management
        </p>
        {links.map((link) => (
          <NavLink
          ...
          >
            ...
            {link.count > 0 && (
              <span className="bg-navy-700 text-white text-[11px] font-bold px-2 py-0.5 rounded-full min-w-[22px] text-center">
                {link.count}
              </span>
            )}
          </NavLink>
        ...
        <div className="pt-6">
          ...
          <div className="sidebar-link text-navy-400 cursor-default">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" />
            </svg>
            <span>Metrics</span>
            <span className="text-[10px] text-navy-300 ml-auto">Soon</span>
          </div>
          <div className="sidebar-link text-navy-400 cursor-default">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
